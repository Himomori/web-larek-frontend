...
### Интерфейс `IProduct`
Описание товара: заголовок, описание, картинка, категория, цена, айди.
...
### Интерфейс `IFormInputChangeData`
Описывает данные которые поменялись при изменении инпута на форме

### Интерфейс `IFormPaymentMethodChangeData`
Описывает данные которые поменялись при изменении способа оплаты на форме
